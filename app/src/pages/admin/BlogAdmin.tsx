...
function BlogAdmin() {
  ...
  const [formData, setFormData] = useState({ title: '', slug: '', content: '', excerpt: '', imageUrl: '', tags: '', author: 'Admin', publishedAt: new Date().toISOString().split('T')[0] });
  const [error, setError] = useState<string | null>(null);

  const loadItems = () => {
    api.getBlog({ limit: 100 })
      .then(r => { setItems(r.items); setLoading(false); })
      .catch(() => { setLoading(false); setError('Failed to load blog posts'); });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const data: Partial<BlogPost> = { ...formData, tags: formData.tags.split(',').map(t => t.trim()).filter(Boolean).join(', '), publishedAt: new Date(formData.publishedAt).toISOString() };
      if (editingId) {
      ...
      setShowForm(false);
      setEditingId(null);
      setFormData({ title: '', slug: '', content: '', excerpt: '', imageUrl: '', tags: '', author: 'Admin', publishedAt: new Date().toISOString().split('T')[0] });
      setError(null);
      loadItems();
    } catch (err) {
  ...
  const handleEdit = (item: BlogPost) => {
    ...
    setFormData({ title: item.title, slug: item.slug, content: item.content, excerpt: item.excerpt || '', imageUrl: item.imageUrl || '', tags: Array.isArray(item.tags) ? item.tags.join(', ') : (item.tags || ''), author: item.author, publishedAt: item.publishedAt.split('T')[0] });
    setShowForm(true);
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, margin: 0 }}>Blog Posts</h1>
          <p style={{ color: 'var(--color-text-muted)', margin: '4px 0 0' }}>{items.length} posts</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setShowForm(true); setEditingId(null); setFormData({ title: '', slug: '', content: '', excerpt: '', imageUrl: '', tags: '', author: 'Admin', publishedAt: new Date().toISOString().split('T')[0] }); setError(null); }}>
          + New Post
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', marginBottom: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <h3 style={{ margin: '0 0 16px' }}>{editingId ? 'Edit Post' : 'New Post'}</h3>
          <div style={{ display: 'grid', gap: '12px' }}>
            ...
            <input className="input" placeholder="Tags (comma-separated)" value={formData.tags} onChange={e => setFormData({ ...formData, tags: e.target.value })} />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8f8f8', borderBottom: '1px solid var(--color-border)' }}>
              ...
              <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600 }}>Tags</th>
